    ...
    if (data) {
      ...
      setActiveTab('introduction'); // Reset tab on module change
       ...
       setReflectionText(''); // Reset reflection text for now
        ...
        // Since this page is statically generated, redirecting client-side might not be ideal
        // Handle invalid ID case appropriately (e.g., show a 404 message or redirect to modules list)
        // For static export, ensure all possible module IDs are generated by generateStaticParams
        // If an invalid ID is somehow accessed, show a clear message.
        setCurrentModule(null); // Set to null to trigger loading/error state below
    }
  }, [moduleId]); // Removed router from dependencies as client-side redirect on initial load is less suitable for static export

  // Potential: Save reflection to localStorage on change
   // useEffect(() => {
   //   if (currentModule?.activity?.saveKey && typeof window !== 'undefined') { // Check for window object
   //     localStorage.setItem(currentModule.activity.saveKey, reflectionText);
   //   }
   // }, [reflectionText, currentModule]);

  if (!currentModule) {
    // Add a check for invalid module ID scenario after initial load attempt
    if(params.moduleId && !moduleData[params.moduleId as string]) {
        return <div className="container mx-auto px-4 py-8 text-center font-open-sans text-muted-foreground">Módulo no encontrado.</div>;
    }
